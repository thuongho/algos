...
function colorGraph(graph, colors) {
  ...
  for (let node of graph) {
    // loop graph throw error
    if (node.neighbors.has(node))
      throw new Error('legal color impossible with loop graph');
    const takenColors = new Set();
    // look for neighbor colors
    for (let neighbor of node.neighbors) {
      if (neighbor.color) takenColors.add(neighbor.color);
    }

    // set color
    node.color = colors.find((color) => !takenColors.has(color));
  }
}
