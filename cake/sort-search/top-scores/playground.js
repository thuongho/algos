...
function sortScores(unorderedScores, highestPossibleScore) {
  if (!unorderedScores || highestPossibleScore === undefined) return;

  // Sort the scores in O(n) time
  const allPossibleScores = buildAllPossibleScoresArr(highestPossibleScore);

  unorderedScores.forEach((score) => {
    allPossibleScores[score - 1] += 1;
  });

  const sortedScores = [];
  for (let i = highestPossibleScore - 1; i >= 0; i--) {
    const scoreGroup = allPossibleScores[i];
    if (scoreGroup > 0) {
      const score = i + 1;
      sortedScores.push(...new Array(scoreGroup).fill(score));
    }
  }
  return sortedScores;
}

function buildAllPossibleScoresArr(n) {
